...
function showBankDetails() {
  ...
  // KEEP bank details visible
  showBankDetails();

  paymentOptions.forEach(option => {
    option.style.display =
      option.getAttribute("data-type") === "pickup"
        ? "block"
        : "none";
  });
  ...
  deliveryNote.innerHTML =
    currentLang === "en"
      ? "Delivery fee not included<br><br>Customer must book Yandex.<br>Address: 31 Shalva Nutsubidze<br>Phone: +995501002006"
      : "მიწოდების საფასური არ შედის<br><br>კლიენტმა თავად უნდა გამოიძახოს Yandex.<br>მისამართი: შალვა ნუცუბიძის 31<br>ტელეფონი: +995501002006";

  paymentOptions.forEach(option => {
    option.style.display =
      option.getAttribute("data-type") === "delivery"
        ? "block"
        : "none";
  });
  ...
  showBankDetails(); // always visible
   });
